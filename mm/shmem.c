...
static int shmem_writepage(struct page *page, struct writeback_control *wbc)
{
	...
	swap = folio_alloc_swap(folio, NULL, false, true);
	if (!swap.val)
		goto redirty;
			     ...
			     vm_fault_t *fault_type)
{
	...
	*foliop = NULL;
	...
	folio = swap_cache_get_folio(si, swap, NULL, 0);
	if (!folio) {
	...
	if (unlikely(entry_is_entry_ext(shadow)==1)){
		pr_info("skip shadow_ext[%p], folio[%p]", shadow, folio);
		shadow_entry_free(shadow);
		trace_shadow_entry_free(shadow, 5);	
	}
	put_swap_device(si);
