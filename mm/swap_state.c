	...
	do {
		...
		for (i = 0; i < nr; i++) {
			...
			if (!folio_test_stalesaved(folio)){
				xas_unlock_irq(&xas);
				pr_err("add_swp_entry_remap abandon folio[%p] st[%d]", folio, folio_test_stalesaved(folio));
				return -EEXIST;
			}
			VM_BUG_ON_FOLIO(xas.xa_index != idx + i, folio);
			...
			swp_entry_t entry, void *shadow)
{
	...
	for (i = 0; i < nr; i++) {
		...
		if (unlikely(entry_ != folio)) {
			pr_err("__delete_sc mismatch entry[%lx]->[%p]<>folio[%p]", 
	...
	*try_free_entry = 1;
	...
	if (!saved_entry.val || non_swap_entry(saved_entry)){ // can't provide now
		...
		do { //continue
			...
			pr_info("add_swp_entry_remap folio[%p] ref[%d][%lx]cnt[%d]=>[%lx][%d]",
...
fail_page_out:
				...
				if (folio_ref_count(folio)){
					pr_err("fail page_out folio[%p], ref err [%d]", folio,folio_ref_count(folio));
					BUG();
				}
				goto skip_this_save;
scceed_pageout:
